...
export async function POST(request: NextRequest) {
    try {
        ...
        const scope = String(form.get('scope') ?? '').trim() as Scope
        ...
        if (scope !== 'home_hero' && scope !== 'category') {
        ...
        const bucket = process.env.SUPABASE_SITE_IMAGES_BUCKET || process.env.SUPABASE_PRODUCT_IMAGES_BUCKET || 'product-images'
        const fileName = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`

        const folder = scope === 'home_hero' ? 'homepage' : `categories/${categoryId}`
        const path = `${folder}/${fileName}`

        const arrayBuffer = await file.arrayBuffer()
        const buffer = Buffer.from(arrayBuffer)

        const { error: uploadError } = await admin.supabase.storage.from(bucket).upload(path, buffer, { contentType, upsert: false })
        if (uploadError) {
        ...
        return NextResponse.json({ success: true, url: data.publicUrl, bucket, path })
    } catch (e) {
